...
### 7.RC、RR级别下的InnoDB的非阻塞读如何实现

1. 数据行里的DB_TRX_ID（最后一个影响改行事务的id）、DB_ROLL_PTR（undo日志的最近记录）、DB_ROW_ID（行id）字段
2. undo日志
3. read view取当行的DB_TRX_ID与系统活跃的事务id做对比，直到undo日志中的DB_TRX_ID小于系统当前活跃的事务id
...
3. 抽象工厂方法
   ...
   一个工厂负责生产多个互相依赖的产品，工厂方法模式一个工厂只负责生产一种产品
...
装饰器模式动态的为对象添加职责，比子类更加灵活。使用对象之间的关联关系取代了继承关系，装饰器设计模式的核心在于其抽象装饰类
